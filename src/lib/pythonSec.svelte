...
<div class="bg-gradient-to-b from-[#3f0f4c] via-[#29148a] to-[#1d58a0]">
  <div
    class="flex flex-col-reverse px-4 pt-28 pr-50 items-center gap-32 md:gap-56 justify-end md:flex-row md:pr-28"
  >
    <div
      class="bg-[#191449] p-10 md:p-14 rounded-lg animate-float md:flex animate-float items-center"
    >
      <p class="font-bold text-white text-center">
        At work I utilize Python to modify datasets<br />At home I write scripts
        to enhance DX
      </p>
    </div>

    <section
      class="flex flex-col w-full border-4 md:w-auto rounded-lg min-w-[340px]"
    >
      <div class=" flex bg-white justify-between">
        <div class="flex gap-2">
          ...
          <input
            type="text"
            readonly
            class="w-1/2 md:w-full hidden md:flex font-bold text-violet-800"
            value="Not scared of Data "
          />
        </div>
      ...
      <div class="imgs w-full">
        ...
        >
          <img
            bind:this={banner}
            src={currentImage}
            alt="Excel-like table"
            class={`min-w-[340px] min-h-[400px] md:w-[600px]   border-4 border-gray-300 shadow-xl ${fadeClass}`}
          />
        </IntersectionObserver>
      </div>
    </section>
